...
!      write(6,*) 'RDP_get:',(rdp(nr),nr=1,nrmax)
      ...
      WRITE(6,'(A,2ES12.4)') 'DR:',DR,RG(2)-RG(1)
      NR=1
      ...
      DO NR=2,NRMAX
         ...
         FACTORP=ABVRHOG(NR  )/TTRHOG(NR  )
!         FACTOR0=TTRHO(NR)/(RMU0*BB*DVRHO(NR))
!         FACTORM=ABVRHOG(NR-1)
!         FACTORP=ABVRHOG(NR  )
         AJ(NR) =FACTOR0*(FACTORP*RDPVRHOG(NR)-FACTORM*RDPVRHOG(NR-1))/DR
         AJOH(NR)=AJ(NR)
         WRITE(6,'(A,I4,6ES12.4)') 'NR:', &
              NR,AJ(NR),FACTORP,FACTORM,RDPVRHOG(NR), &
              FACTORP*RDPVRHOG(NR),FACTORM*RDPVRHOG(NR-1)
      ENDDO
      
      CALL PAGES
      CALL grd1d(1,rg,AJ,nrmax,nrmax,1,'@aj@')
      CALL grd1d(2,rg,DVRHO,nrmax,nrmax,1,'@dvrho@')
      CALL grd1d(3,rg,ABVRHOG,nrmax,nrmax,1,'@abvrhog@')
      CALL grd1d(4,rg,RDPVRHOG,nrmax,nrmax,1,'@rdpvrhog@')
      CALL PAGEE

!      DO NR=1,NRMAX
!         WRITE(6,'(A,I6,4ES12.4)') 'NR,AJ,BP,QP,RDP:', &
!              NR,AJ(NR),BP(NR),QP(NR),RDP(NR)
!      END DO
