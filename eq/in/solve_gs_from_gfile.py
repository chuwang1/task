...
def compute_tj_and_rhs_mdleqf6(R, psi, psi_axis, psi_bdy,
    ...
    # Use ip_integral_mask (= update_mask, a fixed geometric mask excluding
    # the LCFS ring) to avoid psi_n boundary-pixel noise that destabilizes TJ.
    # The RHS source still uses the psi_n-dependent inside mask.
    dA = dr * dz
    if ip_integral_mask is not None:
        # Pure geometric mask — does not depend on current psi field
        ip_mask = ip_integral_mask
    else:
        ip_mask = inside
    # Recompute hjp/hjt1/hjt2 on ip_mask without psi_n gating
    R2d_l = R[:, None] if R.ndim == 1 else R
    hjp_ip = np.where(ip_mask, R2d_l * dPdpsi_2d, 0.0)
    hjt1_ip = np.where(ip_mask, F_vac * dFdpsi_2d / (MU0 * R2d_l), 0.0)
    hjt2_ip = np.where(ip_mask, (F_2d - F_vac) * dFdpsi_2d / (MU0 * R2d_l), 0.0)
    FJP = float(np.sum(hjp_ip) * dA)
    FJT1 = float(np.sum(hjt1_ip) * dA)
    FJT2 = float(np.sum(hjt2_ip) * dA)

    # Ip at TJ=1 gives the natural current direction from the profiles.
    # EQDSK currentA may have a different sign convention, so match signs.
    Ip_at_tj1 = FJP + FJT1 + FJT2
    Ip_signed = float(np.copysign(abs(Ip_target), Ip_at_tj1))

    # Solve quadratic: FJT2*TJ^2 + FJT1*TJ + (FJP - Ip_signed) = 0
    a_coeff = FJT2
    b_coeff = FJT1
    c_coeff = FJP - Ip_signed
    ...
    if abs(a_coeff) < 1e-30:
        ...
        tj1 = (-b_coeff + sqrt_d) / (2.0 * a_coeff)
        tj2 = (-b_coeff - sqrt_d) / (2.0 * a_coeff)
        # Pick the root closest to 1 (physical solution)
        TJ = tj1 if abs(tj1 - 1.0) < abs(tj2 - 1.0) else tj2
...
def solve_gs_fixed_boundary(
    ...
    for k in range(max_outer):
        ...
        if mdleqf6:
            ...
            rhs, TJ_raw, Ip_current, inside, _F2d, _TT2d = \
                compute_tj_and_rhs_mdleqf6(
                    ...
                    dr=dr, dz=dz,
                    ip_integral_mask=update_mask,
                )
            # Relax TJ to suppress oscillation between outer iterations.
            tj_relax = 0.5
            if k == 0:
                TJ_current = TJ_raw
            else:
                TJ_current = tj_relax * TJ_raw + (1.0 - tj_relax) * TJ_current
            # Recompute RHS with relaxed TJ (reuse decomposition arrays)
            P_2d, F_2d, dPdpsi_2d, dFdpsi_2d, inside = interpolate_pf_profiles(
                psi, axis_psi, g.psibdy, psi_n_pf, P_pf, F_pf,
                dPdpsi_pf, dFdpsi_pf,
                lcfs_mask_psi=use_psi_mask,
                lcfs_geom_mask=src_geom_mask if use_lcfs_mask else None,
            )
            R2d = R[:, None]
            hjp_r = np.where(inside, R2d * dPdpsi_2d, 0.0)
            hjt1_r = np.where(inside, F_vac * dFdpsi_2d / (MU0 * R2d), 0.0)
            hjt2_r = np.where(inside, (F_2d - F_vac) * dFdpsi_2d / (MU0 * R2d), 0.0)
            jphi_r = hjp_r + TJ_current * hjt1_r + TJ_current ** 2 * hjt2_r
            rhs = source_scale * (-MU0 * R2d * jphi_r)
            Ip_current = float(np.sum(jphi_r) * dr * dz)
