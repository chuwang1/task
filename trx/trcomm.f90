...
!       NS_e,NS_D,NS_T,NS_He4,NS_H,NS_He3,NS_He5,NS_C,NS_Fe
  ...
  INTEGER:: model_arfixed  ! 0: use PNAR, 1: read Ar density from CSV file
  INTEGER:: model_radfixed ! 0: use calculated radiation, 1: read qrad from CSV file
  INTEGER:: model_prlfixed ! 0: use calculated PRL, 1: read PRL from CSV file
  INTEGER:: model_prffixed ! 0: use Gaussian PRF, 1: read qrfe from CSV file
  ...
  CHARACTER(LEN=128):: knam_arfixed  ! Ar density file name (CSV format)
  CHARACTER(LEN=128):: knam_radfixed ! radiation profile file name (CSV format)
  CHARACTER(LEN=128):: knam_prlfixed ! PRL profile file name (CSV format)
  CHARACTER(LEN=128):: knam_prffixed ! PRF profile file name (CSV format)
  CHARACTER(LEN=128):: knam_chifixed ! chi file name (space-separated format)

  ! === impurity and neutral parameters ===

  INTEGER:: MDLIMP,MDLNI
  REAL(rkind):: PNC,PNFE,PNAR,PNNU,PNNUS
  ...
  INTEGER:: nnbmax,nraymax_nnb_max
  INTEGER,DIMENSION(nnbm):: &
       model_nnb,ns_nnb,nraymax_nnb
  REAL(rkind),DIMENSION(nnbm):: &
       PNBIN,PNBR0,PNBRW,PNBCD,PNBVY,PNBVW,PNBENG,PNBRTG
  REAL(rkind),DIMENSION(:,:),ALLOCATABLE:: PNB_weight
  REAL,DIMENSION(:,:),ALLOCATABLE:: GPNB
...
! TRSRC
  ...
  REAL(rkind), DIMENSION(:),     ALLOCATABLE :: & ! (NRM)
        ...
        POH, PRB, PRC, PRL, PRL_ext, PRSUM, PRSUM_ext, PRF_ext, &
        PCX, PIE, QEI, SIE, SCX, TSIE, TSCX
...
! TRINS
  ...
  SUBROUTINE allocate_trcomm(ierr)
    ...
    ALLOCATE(SPSC(NRMAX,NSMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(POH(NRMAX),PRB(NRMAX),PRC(NRMAX),PRSUM(NRMAX),PRSUM_ext(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PRL(NRMAX),PRL_ext(NRMAX),PCX(NRMAX),PIE(NRMAX),QEI(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(SPE(NRMAX,NSTM),PRF(NRMAX,NSTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PRF_ext(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
 ...
 900 continue
  ...
  SUBROUTINE DEALLOCATE_TRCOMM
    ...
    IF(ALLOCATED(PNB_weight)) DEALLOCATE(PNB_weight)
    ...
    DEALLOCATE(POH,PRB,PRC,PRL,PRL_ext,PRSUM,PRSUM_ext,PCX,PIE,QEI)
    DEALLOCATE(AJNB_NNBNR,PEC_NEC,PLH_NLH,PIC_NIC)
    DEALLOCATE(SIE,SCX,TSIE,TSCX,PIN,SSIN,PBCL,SPE,SPE_NSNPELNR)
    DEALLOCATE(PFCL,PRF,PRF_ext,PRFV,AJRFV,RGFLX)
  ...
  SUBROUTINE DEALLOCATE_ERR_TRCOMM
    ...
    IF(ALLOCATED(ANAR     ))     DEALLOCATE(ANAR     )
    ...
    IF(ALLOCATED(PRSUM_ext))     DEALLOCATE(PRSUM_ext)
