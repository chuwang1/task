...
MODULE trparm
  ...
  NAMELIST /TR/ &
       ...
       model_arfixed,knam_arfixed, &
       model_radfixed,knam_radfixed, &
       model_prlfixed,knam_prlfixed, &
       model_prffixed,knam_prffixed, &
       model_chifixed,knam_chifixed,chifixed_factor, &
        C_SCALING,ALPHA_RELAX,H_FACTOR_USER,C_SCALING_MIN,C_SCALING_MAX, &
        ISCALING_TYPE,HY_CORRECTION_FACTOR,SIGMAV_SCALE_FACTOR, &
...
!     ****** INPUT NAMELIST LINE (internal read) ******

    SUBROUTINE tr_nlin_line(KIN,IST,IERR)

      USE trcomm
      IMPLICIT NONE
      CHARACTER(LEN=*),INTENT(IN) :: KIN
      INTEGER,INTENT(OUT):: IST, IERR
      CHARACTER(LEN=512) :: KNLINE

      KNLINE='&TR '//TRIM(KIN)//' /'
      READ(KNLINE,TR,IOSTAT=IST,ERR=9800,END=9900)
      IST=0
      IERR=0
      RETURN
